...
void showJumble(const charArrayPtr* puzzle, const int size) {
	int i, j;
	cout << "\n  ";
	for (i = 0; i < size; i++)
		if (i < 10)
...
void testJumble() {
	...
	JumblePuzzle* jp;
	string targetWord = "HiddenWord";

	jp = new JumblePuzzle(targetWord, "medium");

	cout << "Testing size of puzzle" << endl;
	int size = jp->getSize();
	int strLen = targetWord.size() * 3; //for medium level
	if (strLen != size)
	cerr << "Invalid size" << endl;

	cout << "Testing directions: " << endl;
	char dir = jp->getDirection();
	if (dir != 'n' &&  dir != 'e' && dir != 's' && dir != 'w')
	cerr << "Invalid direction" << endl;

	if (jp->getRowPos() < 0 || jp->getRowPos() >= size)
	cerr << "Invalid row value" << endl;

	if (jp->getColPos() < 0 || jp->getColPos() >= size )
	cerr << "Invalid col value" << endl;

	cout << "Testing puzzle size for different levels: " << endl;
	targetWord = "tree";
	jp = new JumblePuzzle(targetWord, "easy");//testing for easy
	strLen = targetWord.size() * 2;

	if (jp->getSize() == strLen)
		cout << "Level Easy: correct" << endl;
	else
		cerr << "Level Easy: incorrect" << endl;

	jp = new JumblePuzzle(targetWord, "medium");//testing for medium
	strLen = targetWord.size() * 3;

	if (jp->getSize() == strLen)
		cout << "Level Medium: correct" << endl;
	else
		cerr << "Level Medium: incorrect" << endl;

	jp = new JumblePuzzle(targetWord, "hard");//testing for hard
	strLen = targetWord.size() * 4;

	if (jp->getSize() == strLen)
		cout << "Level Hard: correct" << endl;
	else
		cerr << "Level Hard: incorrect" << endl;


	cout << "Testing Exceptions: " << endl;

	// Testing for invalid target word length <3 || >10
	try{
		jp = new JumblePuzzle("ww", "easy");
	}catch (BadJumbleException& e){
		cerr << e.what() << endl;
	}
	try{
		jp = new JumblePuzzle("wwwwwwwwwww", "easy");
	}catch (BadJumbleException& e){
		cerr << e.what() << endl;
	}

	// Testing for invalid difficulty (must be easy, medium or hard)
	try{
		jp = new JumblePuzzle("word", "Test");
	}
	catch (BadJumbleException& e){
		cerr << e.what() << endl;
	}
	...
	cout << "\nTesting puzzle: " << endl;
	targetWord = "marble";
	jp = new JumblePuzzle(targetWord, "hard");
	int row = jp->getRowPos();
	int col = jp->getColPos();
	dir = jp->getDirection();
	charArrayPtr* puzzle = jp->getJumble();

	if (puzzle[row][col] != targetWord.at(0)){
	...
	for (int i = 1; i < targetWord.size(); i++){
		...
		if (puzzle[row][col] != targetWord.at(i)){
			cerr << "Invalid puzzle" << endl;
		}
	}

	cout << "Puzzle is correct!" << endl;
